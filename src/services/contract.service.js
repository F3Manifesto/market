...
export const getCryptoPaymentTokenContract = async (crypto) => {
  const address = tokens[crypto].address;
  if (!window.web3?.eth) return null;
  const contract = new window.web3.eth.Contract(tokens[crypto].abi, address);
